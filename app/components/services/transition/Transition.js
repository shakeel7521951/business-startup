    ...
    <>
      ...
      <div className="relative w-full sm:h-[500px] h-[350px]">
        ...
        <div className="hidden sm:block absolute bottom-10 left-0 right-0 mx-auto p-8 bg-[#0D1B2A]/50 backdrop-blur-sm rounded-lg shadow-2xl w-full max-w-4xl transform -translate-y-1/2 sm:left-10 sm:right-auto sm:top-1/2 sm:bottom-auto sm:max-w-[50%]">
          ...
          <p className="text-white text-justify text-lg leading-relaxed mb-6">
            ...
            demands, thorough risk assessment to identify and mitigate potential
            disruptions, and strict regulatory compliance to ensure all legal
            requirements are met. The transition phase focuses on change
            management, resource reallocation, and stakeholder communication to
            maintain business continuity while adapting to new environments or
            strategies. Effective transition ensures minimal downtime, preserves
      ...
      <div className="flex justify-center select-none bg-[#F9F9F9] py-12">
        <div className="px-4 sm:px-8 max-w-screen-xl w-full flex flex-col items-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-10 w-full">
            {transitionData.slice(0, visibleCount).map((blog, idx) => (
              ...
              >
                {/* Image */}
                <img
                  src={blog.image || "../services/transition.jpg"}
                  alt={blog.title}
                ...
                <div className="p-6 flex flex-col justify-between flex-grow">
                  ...
                  <div className="mt-5">
                    {idx % 2 === 0 ? (
                      <Link
                        href={`/services/transition/${blog.slug}`}
          ...
          {/* Load More Button */}
          {visibleCount < transitionData.length && (
            <button
              onClick={loadMore}
